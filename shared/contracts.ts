...
export const hideDoneInput = z.object({ ids: z.array(z.string().min(1)).min(1) });
export const hideDoneCards = defineRpc({ name: 'board.hide-done', input: hideDoneInput, output: z.object({ ok: z.boolean(), count: z.number().int().nonnegative() }) });
...
export type HideDoneInput = z.infer<typeof hideDoneInput>;
