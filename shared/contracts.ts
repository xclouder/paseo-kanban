...
export const archiveDoneInput = z.object({ ids: z.array(z.string().min(1)).min(1) });
export const archiveDoneCards = defineRpc({ name: 'board.archive-done', input: archiveDoneInput, output: z.object({ ok: z.boolean(), count: z.number().int().nonnegative() }) });
...
export type ArchiveDoneInput = z.infer<typeof archiveDoneInput>;
