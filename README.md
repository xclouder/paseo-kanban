...
- `npm test`：57 个核心测试，覆盖全局看板与 Inbox 快捷键、Inbox 捕获 / 去重 / 原子转任务、分页、真实 RPC 校验、状态转换、批量完成的原子性与大列表输入、数据保存、损坏保护、附件持久化 / 追加 / 失败清理 / 启动转交 / 删除恢复与路径保护、未启动任务工作区修改、任务创建与 Agent 启动去重 / 响应丢失恢复、运行模式与 Thinking Mode 的默认值、校验及持久化、项目排序与分组的旧数据兼容 / 并发保存，以及真实 SDK 的参数转换。
- `npm run test:e2e`：28 个浏览器场景，覆盖 Inbox 全局捕获、看板挂载前后的主题一致性、焦点管理及转任务、搜索、创建 / 编辑 / 启动、工作区二级选择、未保存草稿关闭确认、附件读取期间的关闭保护、附件拖放 / 粘贴 / 详情展示与未启动任务删除、未启动任务工作区修改、默认模型、运行模式与 Thinking Mode 选择、模型选项回退及旧任务兼容、全屏看板返回入口、Paseo 主侧栏自动收起与恢复、侧栏缩放、单个及批量完成、拖动 / 收起 / 恢复、窄屏与运行中完成保护，以及项目右键菜单定位、项目 / 分组拖动排序、跨组移动、分组编辑与刷新恢复。
