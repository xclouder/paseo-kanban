...
- Inbox 支持先收集、后整理：在 Paseo 任意页面按 `Ctrl+Shift+I`，或在桌面端 Inbox 面板点击“新建想法”，即可打开全局快速输入弹框；桌面端可直接粘贴图片或文件，随后在 Inbox 中补全工作区、模型等信息并创建任务。任务标题和说明默认使用想法名称，附件会随任务保留，任务创建成功后对应 Inbox 条目才会移除。
